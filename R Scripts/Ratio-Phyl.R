...
library(dplyr)
	...
	data <- read.delim('/Users/mariohg/Dropbox/Norma/data/relabavg/ratio_L2_databases.txt', comment.char = "#")
...
# bacteria
	...
	ggplot(data1, aes(x=taxonomy, y=ratio)) +
		...
		theme(axis.text.y = element_text(hjust = 1, size = 11, color = "black"),#, colour = "black"), #tick labels
...
# protoz
	...
	protoz <- 
		...
		geom_point( size=0.1, fill = "black", color="black", shape=21, stroke=2) +
		scale_x_discrete(limits=c(as.character(data1$taxonomy))) +
		scale_y_continuous(limits = c(-0.1, .20), breaks = seq(-0.1, .2, by= 0.1)) + #defining ratio axis values
		theme_bw() + #theme_light() +
		theme(axis.text.y = element_text(hjust = 1, size = 11, color = "black"),#, colour = "black"), #tick labels
		      axis.text.x= element_text(colour ="black", size = 11, color = "black"), #tick labels
		      legend.position = "none") + #panel.border = element_blank()
...
# virus: the 50 most abundant families across treatments
	...
	virus <- 
		...
		geom_point( size=0.1, fill = "black", color="black", shape=21, stroke=2) +
		scale_x_discrete(limits=c(as.character(data1$taxonomy))) +
		scale_y_continuous(limits = c(-0.3, 0.3), breaks = seq(-0.3, 0.3, by = .2)) + #defining ratio axis values
		theme_bw() + #theme_light() +
...
## Joining the plots
	...
	small_ones <- cowplot::plot_grid(arch, fungi, protoz, labels = c("b)", "c)", "d)"), label_size = 12, ncol = 1, align = "v")

	# Joining the entire picture:
	cowplot::plot_grid(bact, small_ones, virus, labels = c("a)", "b)", "e)"), 
