	...
	# average and sd
	data$UNS <- rowMeans(data[,6:7]);
	data$MAS <- rowMeans(data[,8:9])
	data$UNSsd <- apply(data[,6:7], 1, sd); 
	data$MASsd <- apply(data[,8:9],1, sd)
	...
	row.data <- data.frame("Order" = data$Level_4, data$Level_6, row.names = 2); head(row.data)
	...
	data_mat <- data %>% dplyr::arrange(desc(apply(X =data[,6:9], 1, mean)))
